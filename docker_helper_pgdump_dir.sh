...
TMP_DUMP_DIR="tmp_dump_$CURRENT_DATETIME"
...
docker run --rm -it -v `pwd`:/tmp/export/data \
    ...
    /bin/bash -c "export PGPASSWORD=${DB_PASSWORD} && cd /tmp/export/data \
        ...
        && pg_dump -h dbhost -U ${DB_USER} ${DB_NAME} ${PG_DUMP_OPTIONS} -F d -f $TMP_DUMP_DIR \
        && tar -cvf ${CONTAINER_NAME}-${DB_NAME}-${CURRENT_DATETIME}.tar.gz $TMP_DUMP_DIR \
        && chmod 0766 ${CONTAINER_NAME}-${DB_NAME}-${CURRENT_DATETIME}.tar.gz \
        && rm -r $TMP_DUMP_DIR"

printf "${NETWORK}\n${CONTAINER_NAME}\n${CONTAINER_NAME}-${DB_NAME}-${CURRENT_DATETIME}.tar.gz\n" >> ${CONTAINER_NAME}.txt
